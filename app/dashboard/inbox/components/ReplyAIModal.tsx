...
export default function ReplyAIModal({ isOpen, onClose, style, coachMessage, firstMessage }: ModalProps) {
  ...
  const replyAIMain = async () => {
    ...
    if (error) {
      ...
      if (category !== "other") {
        ...
        if (error) {
          ...
          reader.onload = async (event) => {
            ...
            if (fileContent) {
              ...
              replyAI(classification, fileContent, category);
            }
          }
        ...
        replyAI(classification, "", category);
      }
  ...
  const replyAI = async (classification: { predicted_class: any, score: any, category: any, }, fileContent: string, category: string) => {
